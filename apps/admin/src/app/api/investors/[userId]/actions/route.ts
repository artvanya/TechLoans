...
export async function POST(
  ...
  if (action === 'restrict') {
    await prisma.user.update({ where: { id: userId }, data: { isActive: false, lockReason: reason } })
    await writeAuditLog({ actorId: operatorId, actorEmail: session.user.email!, action: 'ACCOUNT_RESTRICT', entityType: 'User', entityId: userId, metadata: { reason } })
    return NextResponse.json({ success: true, data: { action: 'restricted' } })
  }

  if (action === 'restore') {
    await prisma.user.update({ where: { id: userId }, data: { isActive: true, isLocked: false, failedLoginCount: 0, lockReason: null } })
    await writeAuditLog({ actorId: operatorId, actorEmail: session.user.email!, action: 'ACCOUNT_RESTORE', entityType: 'User', entityId: userId })
    return NextResponse.json({ success: true, data: { action: 'restored' } })
  }

  if (action === 'unlock') {
    await prisma.user.update({ where: { id: userId }, data: { isLocked: false, failedLoginCount: 0, lockReason: null } })
    await writeAuditLog({ actorId: operatorId, actorEmail: session.user.email!, action: 'ACCOUNT_RESTORE', entityType: 'User', entityId: userId, metadata: { reason: 'Manual unlock' } })
  ...
  if (action === 'set_tier') {
    ...
    await prisma.investorProfile.update({ where: { userId }, data: { tier } })
    await writeAuditLog({ actorId: operatorId, actorEmail: session.user.email!, action: 'UPDATE', entityType: 'InvestorProfile', entityId: userId, afterState: { tier } })

...
import { InvestmentEditForm } from '@/components/investment-edit-form'
  ...
  return (
    <div style={{ fontFamily: "'Outfit', sans-serif", color: '#E8E6DF', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        {deal.isPortfolio && (
          <span style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '1.5px', padding: '5px 12px', borderRadius: '6px', background: 'rgba(196,163,85,0.1)', color: '#C4A355', border: '1px solid rgba(196,163,85,0.2)' }}>
            PORTFOLIO ENTRY
          </span>
        )}
        {!deal.isPortfolio && deal.openForInvestment && (
          <span style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '1.5px', padding: '5px 12px', borderRadius: '6px', background: 'rgba(44,200,154,0.1)', color: '#2CC89A', border: '1px solid rgba(44,200,154,0.2)' }}>
            OPEN INVESTMENT
          </span>
        )}
      </div>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '1px', background: 'rgba(255,255,255,0.07)', borderRadius: '12px', overflow: 'hidden' }}>
            {[
              ...
              { label: 'Rate (p.a.)', value: <span style={{ color: '#2CC89A' }}>{formatPercent(Number(deal.investorApr))}</span> },
              ...
              <tbody>
                {deal.investments.length === 0 ? (
                  ...
                  <tr key={inv.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                    ...
                    <td style={{ padding: '10px 14px' }}><span style={{ background: 'rgba(191,160,99,0.1)', color: '#BFA063', fontSize: '10px', fontWeight: 600, padding: '2px 7px', borderRadius: '5px' }}>{inv.source ?? 'manual'}</span></td>
                    <td style={{ padding: '10px 14px' }}><span style={{ background: 'rgba(44,200,154,0.1)', color: '#2CC89A', fontSize: '10px', fontWeight: 600, padding: '2px 7px', borderRadius: '5px' }}>{inv.status}</span></td>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '1px', background: 'rgba(255,255,255,0.07)', borderRadius: '12px', overflow: 'hidden' }}>
            {[
              ...
              { label: 'LTV', value: <span style={{ color: Number(deal.ltv) > 72 ? '#E05C5C' : '' }}>{formatPercent(Number(deal.ltv), 0)}</span> },
              { label: 'Rate (p.a.)', value: <span style={{ color: '#2CC89A' }}>{formatPercent(Number(deal.investorApr))}</span> },
              { label: 'Originated', value: deal.originationDate ? formatDate(deal.originationDate.toISOString(), 'short') : '—' },
              { label: 'Closed', value: deal.actualClosingDate ? formatDate(deal.actualClosingDate.toISOString(), 'short') : deal.status === 'ACTIVE' ? <span style={{ color: '#E8A030' }}>Ongoing</span> : '—' },
            ].map(({ label, value }) => (
              <div key={label} style={{ background: '#0F1012', padding: '14px', textAlign: 'center' }}>
                <div style={{ fontSize: '9px', letterSpacing: '1px', textTransform: 'uppercase', color: '#7C7A74', marginBottom: '6px' }}>{label}</div>
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '16px', fontWeight: 500 }}>{value}</div>
          ...
          <PortfolioEditForm deal={{
            ...
            investorApr: Number(deal.investorApr),
            loanDurationMonths: deal.loanDurationMonths,
            actualDurationMonths: deal.actualDurationMonths ?? null,
            originationDate: deal.originationDate ? deal.originationDate.toISOString().slice(0,10) : '',
            actualClosingDate: deal.actualClosingDate ? deal.actualClosingDate.toISOString().slice(0,10) : '',
            borrowerType: deal.borrowerType ?? '',
            borrowerPurpose: deal.borrowerPurpose ?? '',
            totalDealIncome: deal.totalDealIncome ?? '',
            investorIncome: deal.investorIncome ?? '',
            companyIncome: deal.companyIncome ?? '',
            wasDefaulted: deal.wasDefaulted,
            collectionStartDate: deal.collectionStartDate ?? '',
            propertyRealizationPeriod: deal.propertyRealizationPeriod ?? '',
            propertySalePrice: deal.propertySalePrice ?? '',
            fundsDistribution: deal.fundsDistribution ?? '',
            dealComment: deal.dealComment ?? '',
          }} />
        </>
      ) : (
        <>
          {/* Standard deal metrics bar */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '1px', background: 'rgba(255,255,255,0.07)', borderRadius: '12px', overflow: 'hidden' }}>
            {[
              { label: 'Status', value: <span style={{ background: `${statusColors[deal.status] ?? '#7C7A74'}18`, color: statusColors[deal.status] ?? '#7C7A74', fontSize: '11px', fontWeight: 600, padding: '3px 9px', borderRadius: '5px' }}>{deal.status}</span> },
              { label: 'Loan Amount', value: formatCurrency(Number(deal.loanAmount), 'GBP', true) },
